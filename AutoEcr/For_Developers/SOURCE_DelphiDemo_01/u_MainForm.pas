	...
	try
		try
			...
			begin
				...
				myRow := ' ' + 'output param[' + i.ToString + ']';
				myRow := myRow.PadRight(42, ' ') + ' = ' + Value;
...
var
	...
	begin
		...
		Result := Result + Day.ToString.PadLeft(2, '0') + '-';
		Result := Result + Month.ToString.PadLeft(2, '0') + '-';
		Result := Result + Year.ToString.PadLeft(2, '0') + ' ';
...
begin
	...
	try
		...
		begin
			...
			tmpString := tmpString.PadRight(42, ' ') + dude.get_SystemInfo(tmpList.Strings[i]);
...
begin
	ErrorCode := -1;
	if not OpenPictureDialog1.Execute(fm_MainForm.Handle) then Exit;
		...
		case afterCase of
			...
			ac_AfterSettingsChange, ac_AfterStart_Server:
				begin
					...
					ed_TrackingModeRowLimit.Text := dude.trackingMode_RowLimit.ToString;
