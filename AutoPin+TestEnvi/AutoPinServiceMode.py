...
async def activate_window_and_enter_pin(window: pyautogui.Window, pin_number: str) -> None:
    try:
        ...
        await asyncio.sleep(1)  # Allow time for window to become active
        ...
        if window.isActive:
            ...
            logging.warning("Failed to activate the window.")
...
def create_config_if_missing(config_path: str) -> None:
    if not os.path.exists(config_path):
        ...
        config['Settings'] = {'pin': '123456789', 'WindowTitle': 'Windows Security', 'ExeMonitorPath': 'C:\\Windows\\SysWOW64\\CredentialUIBroker.exe'}
